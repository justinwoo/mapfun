...
/***/ function(module, exports, __webpack_require__) {
	...
	var GOOGLE_SHIT = 'AIzaSyC_5e-67fDS6lb593qvW-nN6uVN84ncpTE';
	...
	function locationsMaps(locations) {
	  var children = locations.map(function (location) {
	    return (0, _cycleDom.h)('div', { key: location.id, style: { display: 'inline-block' } }, [(0, _cycleDom.h)('iframe', {
	      width: 300,
	      height: 300,
	      frameborder: 0,
	      style: {
	  ...
	  return (0, _cycleDom.h)('div', children);
	}
	
	function locationsList(locations) {
	  var children = locations.map(function (location) {
	    return (0, _cycleDom.h)('div', [(0, _cycleDom.h)('span', location.name), (0, _cycleDom.h)('span', { style: { color: 'transparent' } }, '_'), (0, _cycleDom.h)('button.delete-location-button', {
	      locationId: location.id,
	      style: { float: 'right' }
	    }, 'X')]);
	  });
	  return (0, _cycleDom.h)('div', { style: { marginBottom: '20px' } }, children);
	}
	
	function view(state) {
	  return (0, _cycleDom.h)('div', [(0, _cycleDom.h)('div', { style: { float: 'left', width: '300px' } }, [(0, _cycleDom.h)('input', { id: 'location-name-input', placeholder: 'enter new location' }), locationsList(state.locations)]), (0, _cycleDom.h)('div', { style: { float: 'right' } }, [locationsMaps(state.locations)])]);
	}
	
	function main(drivers) {
	  var BASE_STATE = {
	    locations: [// List {id: String, name: String}
	    { id: '01', name: 'Helsinki' }, { id: '02', name: 'Washington DC' }, { id: '03', name: 'Hong Kong' }, { id: '04', name: 'Sapporo' }, { id: '05', name: 'Hakodate' }, { id: '06', name: 'Aomori' }, { id: '07', name: 'Tokyo' }, { id: '08', name: 'Nagoya' }, { id: '09', name: 'Kyoto' }, { id: '10', name: 'Osaka' }, { id: '11', name: 'Nagoya' }, { id: '12', name: 'Tokyo' }, { id: '13', name: 'Washington DC' }]
	  };
	
	  var newLocation$ = drivers.DOM.select('#location-name-input').events('keydown').filter(function (e) {
	    return e.keyCode === 13;
	  }).filter(function (e) {
	    ...
	    e.target.value = '';
	    return newLocation;
	  }).map(function (newLocationName) {
	    return function (state) {
	      var newLocations = state.locations.slice();
	      newLocations.push({
	  ...
	  var deleteLocation$ = drivers.DOM.select('.delete-location-button').events('click').map(function (e) {
	    return e.target.locationId;
	  }).map(function (locationId) {
	    return function (state) {
	      var newLocations = state.locations.filter(function (location) {
	  ...
	  var view$ = _rx2['default'].Observable.merge(newLocation$, deleteLocation$).startWith(BASE_STATE).scan(function (a, mapper) {
	    return mapper(a);
	  }).map(function (state) {
	    return view(state);
	  });
